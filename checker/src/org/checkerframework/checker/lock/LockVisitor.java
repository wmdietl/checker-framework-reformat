...
public class LockVisitor extends BaseTypeVisitor<LockAnnotatedTypeFactory> {
    ...
    private void issueErrorIfMoreThanOneLockPreconditionMethodAnnotationPresent(
        ...
        if (lockPreconditionAnnotationCount > 1) {
            checker.report(
                    Result.failure("multiple.lock.precondition.annotations"),
                ...
                if (AnnotationUtils.areSameByClass(annotationMirror, checkerGuardedByClass)) {
                    if (AnnotationUtils.hasElementValue(annotationMirror, "value")) {
                        ...
                        for (String lockExpression : guardedByValue) {
                            preconditions.add(
                                    Pair.of(lockExpression, LockHeld.class.getCanonicalName()));
                        }
                    }
                }
            }
            ...
            if (valueType.hasAnnotation(GuardedBy.class)) {
                ...
                if (!errorKey.equals("argument.type.incompatible")) {
                    ...
                    if (varTypeGuardSatisfiedIndex == -1 && valueTypeGuardSatisfiedIndex == -1) {
                        checker.report(
                                Result.failure(
                                        "guardsatisfied.assignment.disallowed", varType, valueType),
                                valueTree);
                    }
                } else {
    ...
    private void reportFailure(
        ...
        if (overriderLocks == null || overriddenLocks == null) {
            checker.report(
                    Result.failure(
                            messageKey, overriderMeth, overriderTyp, overriddenMeth, overriddenTyp),
                    overriderTree);
        } else {
            checker.report(
                    Result.failure(
    ...
    public Void visitSynchronized(SynchronizedTree node, Void p) {
        ...
        if (enclosingMethod != null) {
            ...
            if (!seaOfContainingMethod.isWeakerThan(SideEffectAnnotation.LOCKINGFREE)) {
                checker.report(
                        Result.failure(
                                "synchronized.block.in.lockingfree.method", seaOfContainingMethod),
                        node);
            }
        }
            ...
            switch (tree.getKind()) {
                case MEMBER_SELECT:
                    if (!isTreeSymbolEffectivelyFinalOrUnmodifiable(tree)) {
                        checker.report(
                                Result.failure("lock.expression.not.final", lockExpressionTree),
                                tree);
                        return;
                    }
                ...
                default:
                    checker.report(
                            Result.failure(
                                    "lock.expression.possibly.not.final", lockExpressionTree),
                            tree);
                    return;
            }
        }
    ...
    private void checkLockExpressionInGuardedByAnnotation(
        ...
        if (enclMethod != null) {
            ...
            flowExprContext =
                    FlowExpressionParseUtil.buildFlowExprContextForDeclaration(
                            enclosingClass, path, checker.getContext());
        }
        ...
        for (String lockExpression : guardedByValue) {
            try {
                ...
                parseExpressionString(
                        lockExpression, flowExprContext, pathForLocalVariableRetrieval, null, tree);
        ...
        if (itselfReceiverMatcher.matches()) {
            ...
            expr =
                    super.parseExpressionString(
                            expression, flowExprContext, path, node, treeForErrorReporting);
        }
