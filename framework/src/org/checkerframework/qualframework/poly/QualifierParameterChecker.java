...
public abstract class QualifierParameterChecker<Q> extends Checker<QualParams<Q>> {
    ...
    public QualifiedTypeFormatter<QualParams<Q>> createQualifiedTypeFormatter() {
        ...
        boolean printVerboseGenerics =
                context.getOptionConfiguration().hasOption("printVerboseGenerics");
        boolean printAllQualifiers =
                context.getOptionConfiguration().hasOption("printAllQualifiers");

        if (context.getOptionConfiguration().hasOption("printQualifierParametersAsAnnotations")) {
            SurfaceSyntaxFormatterConfiguration<Q> config =
                    createSurfaceSyntaxFormatterConfiguration();
            if (config != null) {
                SurfaceSyntaxQualParamsFormatter<Q> formatter =
                        new SurfaceSyntaxQualParamsFormatter<Q>(config);
